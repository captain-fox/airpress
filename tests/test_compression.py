...
def test_checks_signed_pass_package_contains_assets_manifest_and_signature(pkpass_with_assets, cert, key, wwdr_cert):
...
def test_should_compress_signed_pass_package_into_pkpass_archive_using_bytes_method(
    pkpass_with_assets, cert, key, wwdr_cert
):
    pkpass_with_assets.sign(cert=cert, key=key, wwdr=wwdr_cert)
...
def test_should_compress_signed_pass_package_into_pkpass_archive_as_callable(pkpass_with_assets, cert, key, wwdr_cert):
    pkpass_with_assets.sign(cert=cert, key=key, wwdr=wwdr_cert)
