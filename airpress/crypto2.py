...
def salted_hmac(key, salt, value=None):
    # Concatenate key and salt
    key_salt = key + salt

    # Generate the HMAC using SHA-256 hash algorithm
    hmac_value = hmac.new(key_salt, value, hashlib.sha256).digest()
...
def pkcs7_sign(
    ...
    Args:
        certcontent (bytes): Content of pem file certificate
